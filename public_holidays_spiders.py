...
class spider(object):
      ...
      # 取出放假日期范围，返回最小日期和最大日期
      def  changeType(self, year, string):
          ...
          return y1+m1+d1,year+m2+d2 #得出每个节日的最小日期和最大日期

      # 取出调休上班日期，返回日期列表
      def  changeType2(self, year, string):
          string = "".join(re.findall('\d.*', string, re.S))
          if string.find('年') >= 0:
              string = string[5:]
      ...
      def getInfo(self,eachyear):
        ...
        while(i< j):
            ...
            min_date,max_date= self.changeType(year,a[i])
            holiday_list = list(map(int,self.getEveryDay(min_date,max_date)))
        ...
        while (m < j):
            ...
            if checkdata >= 0 or checkdata2 >= 0: #如果该节日没有特殊调休安排则跳过循环
                continue
            work_list = list(map(int,self.changeType2(year,data)))
...
if __name__ == '__main__':
    # 初始化参数
     ...
     # data = pd.read_excel('data/节假日安排爬虫数据.xlsx', sheet_name=str(end_page)) #读取写入结果
     # print(df)
     # print(data)
     print('写入Excel完成！')